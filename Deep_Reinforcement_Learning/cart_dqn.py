...
class DQN(object):
	def __init__(self, D, K, hidden_layer_sizes, gamma, 
		...
		for M2 in hidden_layer_sizes:
			...
			M1 = M2
		self.layers.append(HiddenLayer(M1, K, lambda x: x))
	...
	def train(self, target_network):
		if len(self.experience['s']) < self.min_experiences:
			return
		idx = np.random.choice(len(self.experience['s']), size=self.batch_size,
							   replace=False)
		...
		targets = [r + self.gamma*next_q if not done else r 
				   for r, next_q, done in zip(rewards, next_Q, done)]
		self.session.run(
			self.train_op,
			feed_dict={
				self.X: states,
...
def play_one(env, model, tmodel, eps, gamma, copy_period):
	...
	while not done:
		...
		model.add_experience(prev_s, a, r, s, done)
		model.train(tmodel)
...
if __name__ == '__main__':
	...
	for n in range(N):
		eps = 1.0/np.sqrt(n+1)
		total_rewards.append(play_one(env, model, tmodel, eps, 
									  gamma, copy_period))
		if n % 50 == 0:
			print(n)
